...
const Review = () => {
  ...
  useEffect(() => {
    fetch(
      `https://cloud-kitchen-server-dun.vercel.app/reviews?email=${user?.email}`
    )
      .then((res) => res.json())
  ...
  const handleDelete = (id) => {
    fetch(`https://cloud-kitchen-server-dun.vercel.app/reviews/${id}`, {
      method: "DELETE",
    }).then((data) => {
      console.log(data);
    ...
    <div>
      {reviews.length ? (
        <h2 className="font-bold text-4xl m-3">
          {reviews.length} Review about this food.
        </h2>
      ) : (
